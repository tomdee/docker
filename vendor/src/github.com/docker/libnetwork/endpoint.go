...
func (ep *endpoint) releaseAddress() {
	...
	ipa, err := n.getController().getIPAllocator(n.ipamType)
	if err != nil {
		log.Warnf("Failed to retrieve ip allocator to release interface address on delete of endpoint %s (%s): %v", ep.Name(), ep.ID(), err)
		return
	}
	if err := ipa.ReleaseAddress(ep.iface.poolID, ep.iface.addr.IP); err != nil {
