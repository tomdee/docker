...
func (c *controller) RegisterIpamDriver(name string, ipConfig ipamapi.Config, ipAllocator ipamapi.Allocator) error {
	...
	c.Lock()
	c.ipamDrivers[name] = &ipamData{config: ipConfig, allocator: ipAllocator, defaultLocalAddressSpace: l, defaultGlobalAddressSpace: g}
	c.Unlock()
...
func (c *controller) getIPAM(name string) (id *ipamData, err error) {
	...
	defer c.Unlock()
	var ok bool
	if id, ok = c.ipamDrivers[name]; !ok {
