...
func (i *IpamInfo) UnmarshalJSON(data []byte) error {
	...
	if v, ok := m["IPData"]; ok {
		if err = json.Unmarshal([]byte(v.(string)), &i.IPData); err != nil {
			return err
		}
	}
	return nil
...
func (n *network) ipamAllocate() ([]func(), error) {
	...
	for i, cfg := range n.ipamV4Config {
		...
		d.PoolID, d.Pool, d.Meta, err = ipc.RequestPool(n.addrSpace, cfg.PreferredPool, cfg.SubPool, cfg.Options, cfg.IsV6)
		if err != nil {
			return nil, err
		}
		...
		if cfg.Gateway != "" || d.Gateway == nil {
			if d.Gateway, _, err = ipa.RequestAddress(d.PoolID, net.ParseIP(cfg.Gateway), nil); err != nil {
				return nil, types.InternalErrorf("failed to allocate gateway (%v): %v", cfg.Gateway, err)
			}
		}

		cnl = append(cnl, func() {
			if err := ipa.ReleaseAddress(d.PoolID, d.Gateway.IP); err != nil {
		...
		if cfg.AuxAddresses != nil {
			var ip net.IP
			d.IPData.AuxAddresses = make(map[string]*net.IPNet, len(cfg.AuxAddresses))
			for k, v := range cfg.AuxAddresses {
				...
				if d.IPData.AuxAddresses[k], _, err = ipa.RequestAddress(d.PoolID, net.ParseIP(cfg.Gateway), nil); err != nil {
...
func (n *network) ipamRelease() {
	...
	ipa, err := n.getController().getIPAllocator(n.ipamType)
	if err != nil {
		log.Warnf("Failed to retrieve ip allocator to release address pool(s)'s gateways on delete of network %s (%s): %v", n.Name(), n.ID(), err)
		return
	}
	for _, d := range n.ipamV4Info {
		...
		if d.IPData.AuxAddresses != nil {
			for k, nw := range d.IPData.AuxAddresses {
				if err := ipa.ReleaseAddress(d.PoolID, nw.IP); err != nil {
					log.Warnf("Failed to secondary ip address %s (%v) on delete of network %s (%s): %v", k, nw.IP, n.Name(), n.ID(), err)
				}
			}
		}
		if err := ipc.ReleasePool(d.PoolID); err != nil {
...
func (n *network) getIPv4Data() []driverapi.IPData {
	l := make([]driverapi.IPData, 0, len(n.ipamV4Info))
	n.Lock()
	for _, d := range n.ipamV4Info {
		l = append(l, d.IPData)
	}
...
func (n *network) getIPv6Data() []driverapi.IPData {
	l := make([]driverapi.IPData, 0, len(n.ipamV6Info))
	n.Lock()
...
func (n *network) deriveAddressSpace() (string, error) {
	c := n.getController()
	c.Lock()
	ipc, ok := c.ipamDrivers[n.ipamType]
	c.Unlock()
	if !ok {
